...
const SEARCH_BAR_SCRIPT = `<script>(function(){function init(){try{var sections=document.querySelectorAll('main section.faq');if(!sections.length){setTimeout(init,100);return;}if(document.getElementById('faq-search')){return;}var bar=document.createElement('div');bar.className='faq-search-wrap';bar.innerHTML='<div class="container"><div class="faq-search"><label for="faq-search-input" class="faq-search__label">Search FAQs</label><input id="faq-search-input" type="search" autocomplete="off" placeholder="Search questions and answers\\u2026" class="faq-search__input" aria-label="Search FAQs"/><button type="button" id="faq-search-clear" class="faq-search__clear" aria-label="Clear search" hidden>\\u00d7</button><div class="faq-search__count" id="faq-search-count" aria-live="polite"></div></div></div>';var firstFaq=sections[0];firstFaq.parentNode.insertBefore(bar,firstFaq);var noResults=document.createElement('div');noResults.id='faq-no-results';noResults.className='faq-no-results';noResults.hidden=true;noResults.innerHTML='<div class="container"><div class="faq-no-results__inner"><p class="faq-no-results__msg">No FAQs match your search.</p><p class="faq-no-results__sub">Schedule a complimentary review and ask it directly.</p></div></div>';var lastFaq=sections[sections.length-1];lastFaq.parentNode.insertBefore(noResults,lastFaq.nextSibling);var input=document.getElementById('faq-search-input');var clearBtn=document.getElementById('faq-search-clear');var countEl=document.getElementById('faq-search-count');var allItems=Array.prototype.slice.call(document.querySelectorAll('main section.faq .faq__item'));var allSections=Array.prototype.slice.call(sections);var debounce;function filter(){var q=input.value.trim().toLowerCase();clearBtn.hidden=q.length===0;var matched=0;allItems.forEach(function(item){var qText=(item.querySelector('.faq__q span')||{}).textContent||'';var aText=(item.querySelector('.faq__a')||{}).textContent||'';var hay=(qText+' '+aText).toLowerCase();var hit=q.length===0||hay.indexOf(q)!==-1;item.style.display=hit?'':'none';if(hit){matched++;if(q.length>0){item.open=true;}else{item.open=false;}}});allSections.forEach(function(sec){var visible=sec.querySelectorAll('.faq__item:not([style*="display: none"])').length;sec.style.display=visible===0?'none':'';});noResults.hidden=!(q.length>0&&matched===0);if(q.length===0){countEl.textContent='';}else{countEl.textContent=matched+' '+(matched===1?'match':'matches');}}input.addEventListener('input',function(){clearTimeout(debounce);debounce=setTimeout(filter,80);});clearBtn.addEventListener('click',function(){input.value='';input.focus();filter();});input.addEventListener('keydown',function(e){if(e.key==='Escape'){input.value='';filter();}});}if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',function(){requestAnimationFrame(function(){requestAnimationFrame(init);});});}else{requestAnimationFrame(function(){requestAnimationFrame(init);});}})();</script>`;

const SEARCH_BAR_STYLES = `<style id="faq-search-styles">.faq-search-wrap{padding:1.5rem 0 0;background:#fff;position:sticky;top:0;z-index:30;border-bottom:1px solid rgba(15,23,42,0.06);box-shadow:0 1px 0 rgba(15,23,42,0.04)}.faq-search{max-width:820px;margin-inline:auto;display:flex;align-items:center;gap:.75rem;position:relative;padding-bottom:1.25rem}.faq-search__label{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.faq-search__input{flex:1;padding:14px 44px 14px 18px;border:1px solid #cbd5e1;border-radius:10px;font-size:15px;background:#fff;font-family:inherit;color:#0f172a;outline:none;transition:border-color .15s,box-shadow .15s}.faq-search__input:focus{border-color:#0066cc;box-shadow:0 0 0 3px rgba(0,102,204,0.15)}.faq-search__clear{position:absolute;right:12px;top:50%;transform:translateY(calc(-50% - .625rem));width:28px;height:28px;border:0;background:rgba(15,23,42,0.08);color:#0f172a;border-radius:50%;font-size:18px;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0}.faq-search__clear:hover{background:rgba(15,23,42,0.16)}.faq-search__count{position:absolute;right:0;bottom:-4px;font-size:12px;color:#64748b}.faq-no-results{padding:3rem 0;background:#fafbfc;text-align:center}.faq-no-results__inner{max-width:520px;margin-inline:auto}.faq-no-results__msg{font-size:18px;font-weight:600;color:#0f172a;margin:0 0 .5rem}.faq-no-results__sub{font-size:14px;color:#64748b;margin:0}@media (max-width:640px){.faq-search-wrap{padding-top:1rem}.faq-search{padding-bottom:1.5rem}.faq-search__count{position:static;text-align:right;margin-top:.25rem}}</style>`;
